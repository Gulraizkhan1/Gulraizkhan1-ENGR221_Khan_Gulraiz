...
class GameData:
   ...
   def moveSnakeForward(self, nextCell):
       """ Moves the snake forward into an empty cell.
           The nextCell becomes the new head, and the old tail is removed. """
       # 1. Get the current head and turn it into regular body
       self.getSnakeHead().becomeBody()
      
       # 2. Make the next cell the new head
       nextCell.becomeHead()
       self.__snakeCells.insert(0, nextCell) # Insert at index 0 (new head)
      
       # 3. Clean up the old tail
       oldTail = self.__snakeCells.pop() # Remove from the back
       oldTail.becomeEmpty()
      
   def growSnakeForward(self, nextCell):
       """ Moves the snake forward into a food cell, growing by 1 segment.
           The nextCell becomes the new head, and the tail stays put. """
       # 1. Get current head and turn it into body
       self.getSnakeHead().becomeBody()
      
       # 2. Make food cell the new head
       nextCell.becomeHead()
       self.__snakeCells.insert(0, nextCell)
      
       # 3. Remove the food from tracking list, free up space
