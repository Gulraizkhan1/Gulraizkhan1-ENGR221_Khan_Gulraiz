...
@pytest.mark.custom
def test_bst_lookup_nonexistent_returns_none(nonemptyTree):
    """
    Behavior: Evaluating lookup behavior for a missing key.
    According to the implementation, search() returns None if not found, 
    so lookup should return None rather than crashing.
    """
    assert nonemptyTree.lookup(99) is None

@pytest.mark.custom
def test_bst_insert_duplicate_key(nonemptyTree):
    """
    Behavior: Evaluating how the tree handles duplicate keys.
    In our implementation, if insertKey == node.key, it doesn't match 
    < or >, so it should effectively do nothing (maintaining the original value).
    """
    nonemptyTree.insert(5, "new five")
    # Should still be "five" because we don't have an update case
    assert nonemptyTree.lookup(5) == "five"

@pytest.mark.custom
def test_bst_delete_only_node(emptyTree):
    """
    Behavior: Evaluating deletion of the final node in a tree.
    Verifies that the root correctly reverts to None.
    """
    emptyTree.insert(10, "ten")
    emptyTree.delete(10)
    assert emptyTree.isEmpty()
    assert emptyTree.getRoot() is None

@pytest.mark.custom
def test_bst_delete_exception(emptyTree):
    """
    Behavior: Evaluating error handling when deleting from an empty tree 
     or deleting a key that doesn't exist.
    """
    with pytest.raises(Exception) as e:
        emptyTree.delete(5)
    assert str(e.value) == "Key not in tree."

@pytest.mark.custom
def test_bst_find_successor_single_node():
    """
    Behavior: Evaluating findSuccessor on a tree with only one node.
    The successor of the root in a single-node tree should be the node itself.
    """
    bst = BinarySearchTree()
    bst.insert(100, "century")
    successor = bst.findSuccessor(bst.getRoot())
    assert successor.key == 100
